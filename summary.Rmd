...
df_delta %>% 
  ...
  bold_labels() %>% 
  modify_footnote(everything() ~ NA) %>% 
  modify_header(label = "**Outcome**", stat_0 = "**Mean (SD)**") %>% 
  as_kable_extra()
...
df %>% 
  ...
  scale_y_continuous(transform = scales::pseudo_log_trans(), breaks = c(0, 10, 30, 100, 1000, 3000)) +
  labs(x = "Phase", y = "Roseburia_2 (pseudo-log scale)", color = NULL) +
